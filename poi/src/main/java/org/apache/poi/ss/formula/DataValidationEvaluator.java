 ...
 * {@link DataValidation} instances.
 * Be sure to call {@link #clearAllCachedValues()} if any workbook validation
 * definitions are
 * added, modified, or deleted.
 * <p>
 * Changing cell values should be fine, as long as the corresponding
 * {@link WorkbookEvaluator#clearAllCachedResultValues()}
 * is called as well.
 * 
 */
public class DataValidationEvaluator {
    ...
    /**
     * Use the same formula evaluation context used for other operations, so cell
     * value
     ...
     * 
     * @param cell reference to check - use this in case the cell does not actually
     *             exist yet
     ...
     * 
     * @return returns an unmodifiable {@link List} of {@link ValueEval}s, which may
     *         be empty
     */
    protected static List<ValueEval> getValidationValuesForConstraint(DataValidationContext context) {
        ...
        if (val.getExplicitListValues() != null && val.getExplicitListValues().length > 0) {
            ...
            // note this should return the raw formula result, not the "unwrapped" version
            // that returns a single value.
            ValueEval eval = context.getEvaluator().getWorkbookEvaluator().evaluateList(formula, context.getTarget(),
                    context.getRegion());
            // formula is a StringEval if the validation is by a fixed list. Use the
            // explicit list later.
            // there is no way from the model to tell if the list is fixed values or formula
            // based.
    ...
    /**
     * Use the validation returned by {@link #getValidationForCell(CellReference)}
     * if you
    ...
    public boolean isValidCell(CellReference cellRef) {
        ...
        final Cell cell = SheetUtil.getCell(workbook.getSheet(cellRef.getSheetName()), cellRef.getRow(),
                cellRef.getCol());
    ...
    /**
     * Note that this assumes the cell cached value is up to date and in sync with
     * data edits
     *
     * @param cell The {@link Cell} to check.
     * @param type The {@link CellType} to check for.
     * @return true if the cell or cached cell formula result type match the given
     *         type
     */
    ...
    public static enum ValidationEnum {
        ...
        LIST {
            public boolean isValidValue(Cell cell, DataValidationContext context) {
                ...
                for (ValueEval listVal : valueList) {
                    ValueEval comp = listVal instanceof RefEval
                            ? ((RefEval) listVal).getInnerValueEval(context.getSheetIndex())
                            : listVal;
                    ...
                    if (comp instanceof BoolEval) {
                        if (isType(cell, CellType.BOOLEAN)
                                && ((BoolEval) comp).getBooleanValue() == cell.getBooleanCellValue()) {
                            return true;
                        } else {
                    ...
                    if (comp instanceof NumberEval) {
                        // could this have trouble with double precision/rounding errors and date/time
                        // values?
                        // do we need to allow a "close enough" double fractional range?
                        // I see 17 digits after the decimal separator in XSSF files, and for time
                        // values,
                        ...
                        if (isType(cell, CellType.NUMERIC)
                                && ((NumberEval) comp).getNumberValue() == cell.getNumericCellValue()) {
                            return true;
                        } else {
                            continue; // check the rest
                        }
                    ...
                    if (comp instanceof StringEval) {
                        ...
                        // also, Excel validation is case insensitive - "true" is valid for the list
                        // value "TRUE"
                        if (isType(cell, CellType.STRING)
                                && ((StringEval) comp).getStringValue().equalsIgnoreCase(cell.getStringCellValue())) {
                            return true;
                        } else {
                            continue; // check the rest;
                        }
        ...
        FORMULA {
            /**
             * Note the formula result must either be a boolean result, or anything not in
             * error.
             * If boolean, value must be true to pass, anything else valid is also passing,
             * errors fail.
             * 
            ...
            public boolean isValidValue(Cell cell, DataValidationContext context) {
                ...
                ValueEval comp = context.getEvaluator().getWorkbookEvaluator().evaluate(context.getFormula1(),
         ...
         * We won't re-evaluate cells here. This validation would be after the cell
         * value was updated externally.
         * Excel allows invalid values through methods like copy/paste, and only
         * validates them when the user
        ...
        protected boolean isValidNumericValue(Double value, final DataValidationContext context) {
            try {
                ...
                // per Excel, a blank value for a numeric validation constraint formula
                // validates true
                if (t1 == null)
                    return true;
                Double t2 = null;
                ...
                // one or both formulas are in error, not evaluating to a number, so the
                // validation is false per Excel's behavior.
                return false;
            }
        }
         ...
         * Note that Excel treats validations with constraint formulas that evaluate to
         * null as valid,
         ...
         * @return numeric value or null if not defined or the formula evaluates to an
         *         empty/missing cell.
        ...
        private Double evalOrConstant(String formula, DataValidationContext context) throws NumberFormatException {
            ...
            ValueEval eval = context.getEvaluator().getWorkbookEvaluator().evaluate(formula, context.getTarget(),
                    context.getRegion());
        ...
        /**
         * Validates against the type defined in context, as an index of the enum values
         * array.
         * 
         * @param cell    Cell to check validity of
         * @param context The Data Validation to check against
         * @return true if validation passes
         * @throws ArrayIndexOutOfBoundsException if the constraint type is an invalid
         *                                        index
         */
        public static boolean isValid(Cell cell, DataValidationContext context) {
            return values()[context.getValidation().getValidationConstraint().getValidationType()].isValidValue(cell,
                    context);
        }
    ...
    public static enum OperatorEnum {
         ...
         * @param v1        if null, value assumed invalid, anything passes, per Excel
         *                  behavior
    ...
    /**
     * This class organizes and encapsulates all the pieces of information related
     * to a single
    ...
    public static class DataValidationContext {
         ...
         */
        public DataValidationContext(DataValidation dv, DataValidationEvaluator dve, CellRangeAddressBase region,
